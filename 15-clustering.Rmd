...
```{r}
dat_3 %>%
  ggplot(aes(x = cluster, y = TotalWorkingYears)) +
  geom_boxplot() +
  geom_jitter(data = dat_3, aes(x = cluster, y = TotalWorkingYears, color = Attrition), alpha = 0.2)
```

```{r}
km_aov <- aov(MonthlyIncome ~ cluster, data = dat_3)
anova(km_aov)
#TukeyHSD(km_aov)
```
