...
Another common approach is to take summary statistics and draw your own conclusions. The box plots below show the distribution of the numeric variables. Clusters 3 and 4 similar in almost all of theses attributes.
...
gridExtra::marrangeGrob(g, nrow=2, ncol = 3)
```

You can perform an analysis of variance to confirm. Clusters 3 and 4 do not differ significantly on any of the predictors.

```{r fig.height=6}
km_aov <- map(
  vars_numeric, 
  ~ aov(rlang::eval_tidy(expr(!!sym(.x) ~ cluster)), data = dat_3))
...
map(km_hsd, ~ .x$cluster %>% 
      ...
      filter(rowname == "4-3")) %>% 
  bind_rows() %>%
  bind_cols(predictor = vars_numeric) %>%
  select(predictor, everything())
```

How about the factor variables?

```{r fig.width=6, fig.height=6}
my_barplot <- function(y_var){
  dat_3 %>%
    ggplot(aes(x = cluster, y = !!sym(y_var))) +
    geom_bar() +
#    geom_jitter(data = dat_3, aes(x = cluster, y = !!sym(y_var), color = Attrition), alpha = 0.2) +
    theme_minimal() +
    theme(legend.position = "none") #+
#    labs(title = paste(y_var, "vs cluster"), x = "", y = "")
}
vars_factor <- dat_3 %>% select(-Attrition, -cluster) %>% select_if(is.factor) %>% colnames()

g <- map(vars_factor, my_barplot)
gridExtra::marrangeGrob(g, nrow=2, ncol = 2)
```
