...
```{r}
plot(carseats_mdl_cart2)
```

Let's see the final model.

```{r}
rpart.plot(carseats_mdl_cart2$finalModel)
```

What were the most important variables?

```{r}
plot(varImp(carseats_mdl_cart2), main="Variable Importance with Simple Regression")
```

Evaluate the model by making predictions with the test data set.  

```{r}
carseats_preds_cart2 <- predict(carseats_mdl_cart2, carseats_test, type = "raw")
data.frame(Actual = carseats_test$Sales, Predicted = carseats_preds_cart2) %>%
ggplot(aes(x = Actual, y = Predicted)) +
   geom_point(alpha = 0.6, color = "cadetblue") +
...
```{r}
carseats_rmse_cart2 <- RMSE(
   pred = carseats_preds_cart2,
   obs = carseats_test$Sales
)
carseats_rmse_cart2
```
...
```{r}
scoreboard_r <- rbind(
   data.frame(model = "Manual ANOVA", RMSE = round(carseats_rmse_cart, 5)), 
   data.frame(model = "Caret", RMSE = round(carseats_rmse_cart2, 5))
)
scoreboard_r
```
...
```{r}
rbind(data.frame(model = "Manual ANOVA", RMSE = round(carseats_mdl_cart_pruned_rmse, 5)), 
