...
```{r}
mdls <- list(
   "Single Tree (caret)" = oj_mdl_cart2,
   "Bagged" = oj_mdl_bag,
   "Random Forest" = oj_mdl_rf,
   "GBM" = oj_mdl_gbm
)
# Make predictions on holdout set
holdout_set_pred_probs   <- map(candidate_models, predict, type = "prob", newdata = as.data.frame(d20))
holdout_set_pred_classes <- map(candidate_models, predict, type = "raw", newdata = as.data.frame(d20))
```
