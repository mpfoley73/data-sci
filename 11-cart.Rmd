...
```{r}
scoreboard_r <- rbind(
   data.frame(model = "Manual ANOVA", RMSE = round(cs_rmse_cart, 5)), 
   data.frame(model = "Caret w/tuneGrid", RMSE = round(cs_rmse_cart2, 5))
)
scoreboard_r
```
...
The accuracy is 0.8451 - surprisingly worse than the 0.85915 of the single tree model, but that is a difference of three predictions in a set of 213. 
...
scoreboard <- rbind(
   scoreboard,
   data.frame(model = "Bagging", Acc = round(oj_cm_bag$overall["Accuracy"], 5))
)
scoreboard %>% arrange(desc(Acc))
```
...
oj_mdl_rf <- train(
   ...
   method = "rf",
   metric = "ROC",
   tuneGrid = expand.grid(mtry = 1:7), # searching around mtry=4
   trControl = oj_trControl
)
oj_mdl_rf

# The peak accuracy is at mtry = 3
plot(oj_mdl_rf)
...
scoreboard <- rbind(
   scoreboard,
   data.frame(model = "Random Forest", Acc = round(oj_cm_rf$overall["Accuracy"], 5))
)
scoreboard %>% arrange(desc(Acc))
```

The bagging and random forest models did pretty well, but the manual classification tree is still in first place.  There's still gradient boosting to investigate!
...
# RMSE of 1.9185 is much improved
cs_rmse_bag <- RMSE(pred = cs_preds_bag$Predicted, obs = cs_preds_bag$Actual)

scoreboard_r <- rbind(
   scoreboard_r,
   data.frame(model = "Bagging", RMSE = round(cs_rmse_bag, 5))
)
scoreboard_r %>% arrange(RMSE)
```
...
scoreboard_r <- rbind(
   scoreboard_r,
   data.frame(model = "Random Forest", RMSE = round(cs_rmse_rf, 5))
)
scoreboard_r %>% arrange(RMSE)
```
...
`train()` tuned `n.trees` ($M) and `interaction.depth`, holding `shrinkage = 0.1` (\eta), and `n.minobsinnode = 10`. The optimal hyperparameter values were `n.trees = 200`, and `interaction.depth = 3`.
...
The mean ROC value is the performance measure I used to evaluate the models in `train()` (you can compare the Mean column to each model's object (e.g., `print(oj_mdl_cart2)`)). The best performing model on resamples based on the mean ROC score was GBM. It also had the highest mean sensitivity and specificity. Here is a box plot of the distributions.

```{r}
bwplot(oj_resamples)
```

One way to evaluate the box plots is with a post-hoc test of differences. The single tree was about the same as the random forest and bagged models.
...
The best performing model on resamples based on the mean RMSE score was GBM. It also had the lowest mean absolut error (MAE) and highest R-squared. Here is a box plot of the distributions.

```{r}
bwplot(cs_resamples)
```

The post-hoc test indicates all models differed from each other.
