...
There are three conditions related to study design: 1) the dependent variable is continuous; 2) the independent variable is categorical; and 3) the observations are independent *within* groups. The observations should be from a random sample, or from an experiment using random assignment.  Each group's size should be less than 10% of its population size. The groups must also be independent of each other (non-paired, and non-repeated measures).

There are three conditions related to the data distribution.

1. **Outliers**. There should be no significant outliers in the groups. Outliers exert a large influence on the mean and standard deviation. Test with a box plot.
2. **Normality**.  Each group's values should be *nearly* normally distributed ("nearly" because ANOVA is considered robust to the normality assumption). This condition is especially important with small sample sizes. Test with the Shapiro-Wilk test for normality. If the data is very non-normal, use a nonparametric test such as the Kruskal-Wallis test.
...
The individual populations could be assumed normally distributed if $n_j >= 30$, but $n_j = 6$ for each group, so check for normality.  The QQ plots below appear to be approximately normal.
