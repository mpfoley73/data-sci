...
These notes are primarily taken from studying [PSU STAT-502](https://online.stat.psu.edu/stat502).

Analysis of variance (ANOVA) is a method to compare mean responses grouped by the levels of an experimental treatment factor. The null hypothesis is $H_0: \mu_1 = \mu_2 = \dots = \mu_k$ for the $k$ levels, and the alternative hypothesis is that at least one treatment level mean differs from the others.

## Background

The ANOVA method decomposes the deviation of the $j^{th}$ observation of the $i^{th}$ treatment level around the overall mean $(Y_{ij} - \bar{Y}_{..})$ into the deviation of the factor level mean around the overall mean $(\bar{Y}_{i.} - \bar{Y}_{..})$ plus the deviation of the observation around its factor level mean $(Y_{ij} - \bar{Y}_{i.})$ (aka, the "error" $\epsilon_{ij}$).  
...
A study compares the growth of plants using one of three fertilizers as well as a control group using no fertilizer.  Dataset `greeenhouse` contains 6 observations per each of the *k* = 4 treatment levels (*N* = 24).  

The dataset is formatted as a matrix with the four treatment levels as columns with 6 rows of observations.  The first task is to "gather" the dataset into 24 name-value pair observations.
...
greenhouse <- read_tsv(file = url("https://newonlinecourses.science.psu.edu/stat502/sites/onlinecourses.science.psu.edu.stat502/files/lesson02/lesson1_data/index.txt"))
greenhouse <- greenhouse %>%
  gather(key = group, value = growth) %>%
...
```{r}
pig <- read.delim(file = "Data/pig_weight.txt", header = TRUE, sep = ",")
pig <- pig %>%
