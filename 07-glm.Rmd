...
for each of the $j \in [1, J-1]$ levels of the ordinal outcome variable $Y$ and $p$ predictors. From the **parallel lines assumption**, the intercepts are different for each category but the slopes are constant across categories, and this simplifies the equation above to

$$logit(P(Y \le j)) = \beta_{j0} + \beta_{1}x_1 + \cdots + \beta_{p}x_p.$$

In R, the `polr()` function parameterizes the model with $\eta_i = -\beta_i$, so 

$$logit(P(Y \le j)) = \beta_{j0} - \eta_{1}x_1 - \cdots - \eta_{p}x_p.$$

#### Example {-}

Suppose we want to test whether the binary predictor parental education (`pared`) predicts the ordinal outcome of a student's intention to apply to a college (`apply`) [*unlikely*, *somewhat likely*, *very likely*].

```{r}
dat <- foreign::read.dta("https://stats.idre.ucla.edu/stat/data/ologit.dta")
m <- MASS::polr(apply ~ pared, data = dat)
summary(m)
```

For students whose parents attend college, the log odds of belonging to the base level *unlikely* versus the other levels (*somewhat likely* or *very likely*) is $-\hat{\eta}_1 = -1.127$.
