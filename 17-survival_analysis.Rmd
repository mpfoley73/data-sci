...
The next failure event has a reduced $R_2$ patients at risk.  The partial likelihood for the $D$ failure times is the product $L_i = p_1 p_2 \cdots p_D$. Cox uses maximum partial likelihood estimation to find the value of $\phi$ that maximizes the likelihood function.

Fit a Cox proportional hazards model with `coxph()`. The model below regresses survival on the menopausal status factor variable  `menostat = c(pre, post)` and patient `age`.

```{r}
mdl.cox <- coxph(Surv(time, cens) ~ menostat + age, data = GBSG2)
summary(mdl.cox)
```

The model summary shows that of the 686 patients, there were 299 events (deaths).
