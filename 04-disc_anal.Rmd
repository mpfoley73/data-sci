...
vitc_dat %>% data.frame() %>% rownames_to_column(var = " ") %>%
...
smoke_dat %>% data.frame() %>% rownames_to_column(var = " ") %>%
janitor::adorn_totals(where = c("row", "col"))
```
...
rownames(chd_dat) <- c("CHD", "No CHD")
chd_dat %>% data.frame() %>% rownames_to_column(var = " ") %>%
janitor::adorn_totals(where = c("row", "col"))
```
...
The **chi-square independence test** tests whether observed joint frequency counts $O_{ij}$ differ from expected frequency counts $E_{ij}$ under the *independence model* (the model of independent explanatory variables, $\pi_{ij} = \pi_{i+} \pi_{+j}$. $H_0$ is $O_{ij} = E_{ij}$.  (The *independence model* is a subset of the *saturated model*).

There are two possible test statistics for this test, Pearson $X^2$, and deviance $G^2$.  The sampling distribution of each test statistic approach $\chi^2$ as $n \rightarrow \infty$.
...
$\chi^2$ and $G^2$ increase with the disagreement between the saturated model proportions $p_{ij}$ and the independence model proportions $\pi_{ij}$.
...
(r_std <- (
  E - vitc_dat) / sqrt(E 
  * (1 - prop.table(vitc_dat, margin = 1)) 
  * (1 - prop.table(vitc_dat, margin = 2))
))
