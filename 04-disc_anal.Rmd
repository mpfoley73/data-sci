...
The CHD case, the four levels of cholesterol (0-199, 200-219, 220-259, and 260+) may be treated as ordinal data.  You can also treat the presence of heart disease as ordinal. The Pearson correlation, 
...
y <- c(1:4)
cov(x,y)
cov_xy <- sum((x - sum(x*p)) * (y - sum(y*p)) * chd_o)
s_x <- sum((x - sum(x*p))^2 * chd_o)
s_y <- sum((y - sum(y*p))^2 * chd_o)
r = cov_xy / sqrt(s_x * s_y)
x <- idx
cor(chd_o[1, ])
```
