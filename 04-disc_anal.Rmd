...
The **chi-square goodness-of-fit test** tests whether observed frequency counts $O_j$ of the levels of a categorical variable differ from expected frequency counts $E_j$. $H_0$ is $O_j = E_j$.
...
As $n \rightarrow \infty$, $G^2$ and $X^2$ approach $\chi_{k-1}^2$ where $k$ is the number of n-way table cells.  You can therefore run a chi-square test of significance using the $G^2$ and $\chi^2$ test statistics.  The chi-square test provides reliable results when at least 80% of $E_j >= 5$.  
...
A special case of the one-way table is the $2 \times 1$ table for a binomial random variable.  Here you can hypothesize whether $\pi$ is <, =, or > $\pi_0$, or you can create a confidence interval around the estimate.

Suppose a maintenance company claims to resolve at least 70% of maintenance requests within 24 hours. In a random sample of $n = 50$ repair requests, the company resolves $O_1 = 33$, $p_1 = 66\%)$ within 24 hours. At a 5% level of significance, is the maintenance company’s claim valid?
...
The null hypothesis is that the maintenance company resolves $\pi_0 = 0.70$ of requests within 24 hours, $H_0: \pi_0 = \pi_0$ with alternative hypothesis $H_a: \pi < \pi_0$. This is a left-tailed test with an $\alpha = 0.05$ level of significance.
...
0.12/0.88 = 0.14
odds of "success" (i.e., getting a cold) given that a skier took a placebo pill, 0.22/0.78 = 0.28.
The odds ratio is 0.14/0.28 = 0.49.
