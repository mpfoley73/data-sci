...
Prepare the data by filtering to just the relevant tweets, tokenizing, and creatiing a document-term matrix with TFIDF weighting. If resources are an issue, you may want to reduce terms from the sparse matrix. User `dt::removeSparseTerms()` and experiment with values of parameter `sparse` until you get a term count that is respectable. Below, `sparse = 0.9999` finally gets a respectable term count 25% the size of the orginal count. 
...
tweets_sentences <- tweets %>%
  unnest_tokens(output = "sentence", token = "sentences", input = "content")

# label data
tweets_sentences_2 <- tweets_sentences %>%
  mutate(
    clinton = str_detect(sentence, "clinton"),
    trump = str_detect(sentence, "trump"),
    sentence = str_replace_all(sentence, "clinton", "person X"),
    sentence = str_replace_all(sentence, "trump", "person X"),
    name = as.factor(if_else(clinton, "clinton", "trump")),
    sentence_id = row_number()
  ) %>%
  filter(clinton + trump == 1) %>%
  select(name, sentence_id, external_author_id, region, language, following, followers)

set.seed(1012)
train_ind <- caret::createDataPartition(tweets_sentences_2$name, p = 0.80, list = FALSE)
tweets_train <- tweets_sentences_2[as.integer(train_ind), ]
tweets_test <- tweets_sentences_2[-as.integer(train_ind), ]

rfc <- randomForest::randomForest(x = as.data.frame(as.matrix(tweets_train %>% select(-name))),
                                  y = tweets_train$name,
                                  nTree = 50)

skimr::skim(tweets_train)
```
...
tweets_dtm_2 <- tweets_dtm %>%
  tm::removeSparseTerms(sparse = 0.9999)
```

Split the data set into training and test and model.

```{r}
train_ind <- caret::createDataPartition(tweets_dtm$)
```
