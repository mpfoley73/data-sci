...
Topic modeling is an unsupervised learning approach to discovering abstract "topics" in a collection of documents (tweets, emails, chapters, books, etc.). The most common algorithm is Latent Dirichlet Allocation (LDA). An increasingly popular alternative is Structured Topic Modeling (STM). Most of the preparation and analysis is the same regardless of the algorithm, so these notes present the analysis using both methods side-by-side.
