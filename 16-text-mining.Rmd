...
Text mining is the process of distilling actionable insights from text, typically by identifying patterns with statistical pattern learning. Commont text mining tasks include topic modeling, sentiment analysis, and classification modeling.

Be aware that there are two main approaches to text mining.  *Semantic parsing* identifies words by type and order (sentences, phrases, nouns/verbs, proper nouns, etc.). *Bag of words* simply treats words as attributes of the document. Bag of words is the simpler way to go, the only one used in any of these resources, and the one used throughout these notes.
...
As much as possible, I will build examples using the text from <u>The Adventures of Tom Sawyer</u>, available on https://gutenberg.org and made available in the **gutenbergr** package. I added the chapter identifier and dropped the book id column. Each row is a line of text <=80 characters long.
