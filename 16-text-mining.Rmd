...
Prepare the data by tokenizing, and creating a document-term matrix with TF (term frequency) weighting. In the chunk below, I'm lemmatizing the words to stems, and I'm removing my own list of stop words (e.g., "tom" is so common that I don't want to include it in anything.)
...
sawyer_dtm <- sawyer_tokens %>%
  ...
  cast_dtm(document = chapter, term = word_stem, value = n, weighting = tm::weightTf) %>%
...
There may be an elbow at *k* = 3 topics. Let's assume you've played with different sizes of *k* and 3 is what you settle on. 
...
`topicmodels::LDA()` returns two outputs: a "beta" matrix of probabilities of terms belonging to topics; a "gamma" matrix of probabilities of documents belonging to topics. The tidytext package provides a `tidy()` method for extracting these matrices.

```{r}
sawyer_beta <- sawyer_lda %>%
  tidy(matrix = "beta") 
...
#  arrange(topic, -beta) %>%
  ...
  scale_fill_manual(values = RColorBrewer::brewer.pal(n = 3, name = "Set2"), name = "Topic") +
  scale_x_reordered() +
  coord_flip() +
  facet_wrap(~ topic, scales = "free") +
  labs(title = "LDA Beta Matrix")
```

```{r results='hide'}
colors3 <- RColorBrewer::brewer.pal(n = 3, name = "Set2")
x <- map(c(1:3), ~ with(sawyer_beta %>% filter(topic == .x), 
...
sawyer_lda %>% tidy(matrix = "gamma") %>%
  ggplot(aes(x = as.numeric(document), 
             y = gamma, 
             fill = factor(topic, labels = c("huck", "sid", "joe")))) +
  geom_col() +
  scale_fill_manual(values = RColorBrewer::brewer.pal(n = 3, name = "Set2"), name = "Topic") +
  labs(x = "Chapter", y = "Topic Probability", title = "LDA Gamma Matrix")
```

Topic 1 focuses on Huck; topic 2 on Tom's brother Sid; and topic 3 on Injun Joe. Another way to look at the betas is to identify terms that had the greatest difference in beta between one topic and the next most probable topic. A good way to do this is with the log ratio of the two, $log_2(\beta_2 / \beta_1)$.  Log ratios are useful because the differences are symmetrical ($log_2(2) = 1$, and $log_2(.5) = -1$).  To constrain the analysis to a set of especially relevant words, filter for relatively common words having a beta greater than 1/1000 in at least one topic.
