...
where $\pi$ is the event probability. You can solve the model for $\pi$ to get the event probability:
...
You might use a goodness-of-fit framework to analyze this data.  E.g., you could bin CELL into "low" and "high", breaking at CELL = 0.90, then perform a chi-square test.

```{r}
dat <- leuk %>% mutate(CELL_BIN = if_else(CELL < 0.90, "Low", "High")) 
dat2 <- table(dat$REMISS, dat$CELL_BIN)
chisq.test(dat2)
```

But you might want to model the risk of REMISS given all the predictors, which may be discrete or continuous.  To fit a logistic regression, use `glm(formula, data, family = binomial(link = logit))`.
...
$$logit(\hat{\pi_i}) = 64.3 + 30.8 \hspace{1mm} CELL + 24.7SMEAR - 25INFIL + 4LI = 0.01BLAST = 100 TEMP$$
