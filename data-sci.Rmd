...
The tidy text format is a table with one token (meaningful unit of text, such as a word) per row.  The **tidytext** package assists with the major tasks in text analysis.  A typical text analysis using tidy data principles unnests tokens with `unnest_tokens()`, removes unimportant "stop words" tokens with `anti_join(stop_words)`, and summarizes token counts `count()`.

The **janeaustenr** package contains Jane Austen's complete novels. Each novel is a character vector of ~70 characters. Here are the first few elements of Emma.
...
Unnest tokens with `unnest_tokens()` and specify the type of token with `token = c("words", "sentences", "paragraphs")` and a host of other options.
...
# clever
# [] mean any of, \\d is any number, v, x, l, c are just roman numerals
chapter_regex <- stringr::regex("^chapter [\\divxlc]", ignore_case = TRUE)
austen %>%
  group_by(book) %>%
  mutate(linenumber = row_number(),
         new_chapter = cumsum(str_detect(text, chapter_regex))) %>%
  ungroup() %>%
  unnest_tokens(output = "sentence", input = text, token = "sentences") 
```
