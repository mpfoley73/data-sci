...
<!--chapter:end:02-contingency_tables.Rmd-->
...
<!--chapter:end:04-correlation.Rmd-->
...
<!--chapter:end:14-dimensionality_reduction.Rmd-->
...
<!--
Here is an example using the Russian Troll tweets data set. What are the common themes of tweets that mention "clinton"? Prepare the data by filtering to just the relevant tweets, tokenizing, and creating a document-term matrix with TF (term frequency) weighting. 
...
Create a topic model with `topicmodels::LDA()`.  Parameter `k` specifieds the number of topics. To decide on the number of topics, you can construct a scree plot of the perplexity using the test data set. In general, you only want as many topics as are clearly distinct and that you can easily communicate to others. Here are models with 2:15 topics.
...
k = c(2:15)
perp <- k %>% 
  map(~ topicmodels::LDA(tweets_train, k = .x, method = "Gibbs", control = list(seed = 1234))) %>%
  map(~topicmodels::perplexity(.x, newdata = tweets_test)) %>%
  as.numeric()
data.frame(k = k, y = perp) %>%
