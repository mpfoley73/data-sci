...
Here is a case study to illustrate the points.  Dataset `donner` contains survivorship statistics of 45 members of the Donner party.
...
donner <- tribble(
  ...
  mutate(
    surv = factor(surv, labels = c("Died", "Lived")),
...
The coefficients table "z value" is the Wald z statistic,  $z = \hat{\beta} / SE(\hat{\beta})$, which if squared is the Wald chi-squared statistic, $z^2 = \left(\hat{\beta} / SE(\hat{\beta}) \right)^2$.  The p.value is the area to the right of $z^2$ in the $\chi_1^2$ density curve.
...
The log odds of a 24 year-old female surviving is $\hat{y} = 2.59$.  
...
The log odds of a 24 year-old male surviving is
...
The odds of a female of average age surviving is $\exp(\hat{y}) = 2.93$.  The odds of a male of average age surviving is $\exp(\hat{y}) = 0.49$.
...
The probability of a female of average age surviving is $\pi = 0.75$.  The probability of a female of average age surviving is $\pi = 0.33$.  The `predict()` function for a logistic model returns log-odds, but can also return $\pi$ by specifying parameter `type = "response"`.
...
The odds of a female surviving are multiplied by a factor of $\exp(1 \cdot (-0.19)) = 0.824$. The odds of a male surviving are multiplied by a factor of $\exp(1 \cdot (-0.161-0.19)) = 0.824$.   

```{r}
exp(1 * coef(m)["age"])
exp(1 * coef(m)["age"] + 1*coef(m)["sexM:age"])
```
...
$G^2 = 32.319$ is the change in $G^2$ when adding GRAVITY to the model.
