...
The maximal margin classifier is the hyperplane with the maximum margin, $\max \{M\}$ subject to $||\beta|| = 1$.  A separating hyperplane rarely exists.  In fact, even if a separating hyperplane does exist, its resulting margin is probably undesirably narrow. Here is the maximal margin classifier.^[Image from https://nlp.stanford.edu/IR-book/pdf/15svm.pdf.]

![Maximum marginal classifier](images/svm_mmc.png)

The data set has two linearly separable classes, $y \in [-1, 1]$ described by two features, $X1$ and $X2$^[Any more features and we won't be able to visualize it]. The code is unimportant - just trying to produce the visualization.
