...
These notes rely on [@James2013], [@Hastie2017], and [@Kuhn2016], [PSU STAT 508](https://online.stat.psu.edu/stat508/), and the [e1071 SVM vignette](https://cran.r-project.org/web/packages/e1071/vignettes/svmdoc.pdf).

Support Vector Machines (SVM) is a classification model ^[Other classification models include LDA, logistic regression, and tree-based models such as bagging, random forests, and gradient boosting.] that maps observations as points in space so that the categories are divided by as wide a gap as possible. New observations can then be mapped into that same space for prediction. The SVM algorithm finds the optimal separating hyperplane using a nonlinear mapping to a sufficiently high dimension. The hyperplane is defined by the observations that lie within a margin optimized by a cost hyperparameter. These observations are called the *support vectors*.
